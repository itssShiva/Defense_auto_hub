...
/* ═══════════════════════════════════════════════════════════ */
const EditUsedCar = ({ carId, onCancel }) => {
    const { usedCars, updateUsedCar, fetchAllUsedCars, loading } = useCars();
    const [form, setForm] = useState(null);
    
    // Separate states for images
    ...
    }, [fetchAllUsedCars, usedCars.length]);

    useEffect(() => {
        const car = usedCars.find(c => c._id === carId);
        if (car) {
            setForm({
                brandName: car.brandName || "",
                modelName: car.modelName || "",
                kmTravelled: car.kmTravelled || "",
                year: car.year || "",
                ...
                HPEndorsementFee: car.HPEndorsementFee || "",
                HSRPSMartCardTemporaryFee: car.HSRPSMartCardTemporaryFee || "",
                BHOnRoadPrice: car.BHOnRoadPrice || "",
                ExShowroomPriceBH: car.ExShowroomPriceBH || "",
                BHRegistrationCost: car.BHRegistrationCost || "",
                BHInsurance: car.BHInsurance || "",
                BHRegistrationFee: car.BHRegistrationFee || "",
                BHFastTagFee: car.BHFastTagFee || "",
                BHHPEndorsementFee: car.BHHPEndorsementFee || "",
                BHHSRPSMartCardTemporaryFee: car.BHHSRPSMartCardTemporaryFee || "",
                civilExShowroomPrice: car.civilExShowroomPrice || "",
    ...
    const validate = () => {
        ...
        requiredText.forEach((f) => {
            if (!form[f]?.toString().trim()) newErrors[f] = "This field is required.";
        });
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(validationErrors).length > 0) {
            ...
            document.getElementById(`edit-field-${firstKey}`)?.scrollIntoView({ behavior: "smooth", block: "center" });
            return;
        }
        ...
        // Append JSON array of existing image paths
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-6">
                ...
                <div className="bg-[#fafbf8] p-6 sm:p-8 rounded-2xl border border-[#708ca4]/20 shadow-sm">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <Field label="Model Name" required error={errors.modelName}>
                            <input id="edit-field-modelName" type="text" name="modelName"
                        ...
                        <Field label="Manufacturing Year" required error={errors.year}>
                            <input id="edit-field-year" type="number" name="year"
                        ...
                        <Field label="Color" required error={errors.color}>
                            <input id="edit-field-color" type="text" name="color"
                ...
                <div className="bg-[#fafbf8] p-6 sm:p-8 rounded-2xl border border-[#708ca4]/20 shadow-sm">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <Field label="Owner Name" required error={errors.owner}>
                            <input id="edit-field-owner" type="text" name="owner"
                        ...
                        <Field label="Kilometers Driven" required error={errors.kmTravelled}>
                            <input id="edit-field-kmTravelled" type="number" name="kmTravelled"
                ...
                <div className="bg-[#fafbf8] p-6 sm:p-8 rounded-2xl border border-[#708ca4]/20 shadow-sm">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <Field label="Address" required error={errors.Address}>
                            <input id="edit-field-Address" type="text" name="Address"
                ...
                <div className="bg-[#fafbf8] p-6 sm:p-8 rounded-2xl border border-[#708ca4]/20 shadow-sm">
                    ...
                    {existingImages.length > 0 && (
                        <div className="mb-4">
                            <p className="text-sm font-semibold text-[#19456d] mb-2">Current Images</p>
                    ...
                    {newImagePreviews.length > 0 && (
                        <div className="mb-4">
                            <p className="text-sm font-semibold text-green-600 mb-2">New Images to Upload</p>
                            <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                                {newImagePreviews.map((src, idx) => (
                                    <div key={idx} className="relative group rounded-xl overflow-hidden border border-green-200 aspect-square bg-white">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-xs text-[#708ca4]">
                            Accepted formats: JPG, PNG, WEBP · Max 5 MB per image · Up to 10 images total
                        </p>
                    </div>
                </div>
